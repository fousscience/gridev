...
Guide technique pour configurer le Studio, Resend, les webhooks et le déploiement.

**Pour l’équipe ONG (publier du contenu sans code)** : voir [GUIDE_UTILISATION.md](GUIDE_UTILISATION.md).
...
1. Redémarrer `npm run dev`. Le chat **Sara** s’ouvre automatiquement à la visite (bouton bas à droite aussi).
2. Sans clé, le chat indique que le service n’est pas configuré.
...
Webhook **déjà configuré** sur le projet `sdzup4w2` :

- Nom : `Vercel revalidate`
- URL : `https://gridev-amber.vercel.app/api/revalidate?secret=…` (secret = `SANITY_REVALIDATE_SECRET`)
- Déclencheurs : create / update / delete
- Types : actualite, projet, opportunite, opportunitesPage, about, siteSettings

Vérifier / logs : [https://www.sanity.io/manage/project/sdzup4w2/api/webhooks](https://www.sanity.io/manage/project/sdzup4w2/api/webhooks)

Si tu recrées le webhook manuellement :

1. Générer / réutiliser `SANITY_REVALIDATE_SECRET` (`.env.local` + Vercel).
2. Sanity → API → Webhooks → Create :
  - URL : `https://VOTRE_DOMAINE/api/revalidate?secret=VOTRE_SECRET`
  - Déclencheurs : create / update / delete / publish
3. Après Publish dans le Studio, le site se met à jour en quelques secondes.
4. Tester : publication d’une actualité → la page liste se rafraîchit.
